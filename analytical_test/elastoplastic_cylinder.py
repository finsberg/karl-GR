...
output_dir = Path(
    "results_elastoplastic_cylinder",
...
def analytical_pressure(points, r_i):
    """Analytical solution for pressure as a function of position."""
    # Extract radius R from line_points
    R = points[:, 0]

    # Calculate r(R) = sqrt(r_i^2 + g^2 * (R^2 - R_i^2))
    r = np.sqrt(r_i**2 + g_1 * g_2 * (R**2 - R_i**2))

    p_i = (R_i * g_1 * g_2 / r_i) * (c - mu * (R_i / r[0]) * (g_2 / g_1))

    term1 = (g_1 / g_2) * np.log(R / R_i)
    term2 = -(g_2 / g_1) * np.log(r / r[0])
    term3 = g_2 * (r[0] ** 2 - g_1 * g_2 * R_i**2) / (2 * g_1) * (1 / r**2 - 1 / r[0] ** 2)

    pressure = mu * (term1 + term2 + term3) + p_i

    return pressure


def analytical_stress_nn(points, r_i):
    """Analytical solution for normal (radial) stress."""
    # Calculate radius R for each point
    R = points[:, 0]

    # Calculate r(R) = sqrt(r_i^2 + g^2 * (R^2 - R_i^2))
    r_R = np.sqrt(r_i**2 + g_1 * g_2 * (R**2 - R_i**2))

    p = analytical_pressure(points, r_i)

    # Implement the analytical formula
    term1 = mu * (R / r_R) * (g_2 / g_1)
    term2 = (r_R / (R * g_1 * g_2)) * p

    return term1 + term2


def analytical_stress_ff(points, r_i):
    """Analytical solution for circumferential (tangential) stress."""
    # Extract radius R from line_points
    R = points[:, 0]

    # Calculate r(R) = sqrt(r_i^2 + g^2 * (R^2 - R_i^2))
    r_R = r_i**2 + g_1 * g_2 * (R**2 - R_i**2)
    p = analytical_pressure(points, r_i)

    term1 = mu / (g_2**2 * R**2)
    term2 = p / r_R

    # need to rescale to fit with orthonormal basis
    scale = R * np.sqrt(r_R)

    return (term1 + term2) * scale


def analytical_displacement(points, r_i):
...
def get_deformed_radius(u_func, points):
    """
    Calculates the distance of a deformed point from the origin.

    Args:
        u_func: The dolfinx displacement function.
        points: A numpy array representing the coordinates of the point(s).

    Returns:
        The Euclidean distance of the deformed point(s) from the origin.
    """
    # Ensure points is a 2D array for evaluation
    if points.ndim == 1:
        points = np.array([points])

    # Evaluate displacement at the points
    displacement = scifem.evaluate_function(u_func, points)

    # Calculate deformed coordinates (only in x-y plane for radius)
    deformed_points = points[:, :2] + displacement[:, :2]

    # Calculate the norm (distance from origin)
    deformed_radii = np.linalg.norm(deformed_points, axis=1)

    # Return the scalar value if only one point was given
    return deformed_radii[0] if deformed_radii.shape[0] == 1 else deformed_radii


# Create dictionary of analytical solutions
analytical_solutions = {
    "p": analytical_pressure,
    "stress_nn": analytical_stress_nn,
    "stress_ff": analytical_stress_ff,
    "deformed_radius": analytical_displacement,
}
...
F_tot = ufl.variable(ufl.Identity(3) + ufl.grad(u))

# A = ufl.variable(F)# * ufl.inv(G)
# Isotropic deformation
F_p = g_1 * ufl.outer(geo.n, geo.n) + g_2 * ufl.outer(geo.f, geo.f) + ufl.outer(geo.s, geo.s)

F_e = F_tot * ufl.inv(F_p)

F = ufl.variable(F_e)  # Deformation gradient tensor
...
neumann = ufl.inner(v, traction * N) * ds(  # ufl.det(F_tot) * ufl.inv(F_tot).T *
...
robin = ufl.inner(robin_value * v, N) * ds(  # ufl.det(F_tot) * ufl.inv(F_tot).T *
...
pressure_term = p * (ufl.det(F) - 1) * dx
...
stress = stress + p * ufl.inv(F_tot.T)  # * F_p.T / ufl.det(F_p)
# stress = stress + p * ufl.inv(F_e).T * ufl.det(F_e)   # PK1 stress
...
# Calculate deformed radius for all points on the line and add to history
deformed_radii = get_deformed_radius(u, line_points)
line_history["deformed_radius"] = [deformed_radii]
...
plot_line(r_i=1.101, line_history=line_history, line_points=line_points, analytical_solutions=analytical_solutions)
